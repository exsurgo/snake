...
["", "app.dart", , S, {
  "^": "",
  main: [function() {
    new S.Game(null, null, null, null, null, 0, 15, null, null, 0, false, null, null).Game$0();
  }, "call$0", "main$closure", 0, 0, 0],
  Game: {
    "^": "Object;scoreBoard,content,level,snake,food,score,speed,direction,interval,over,paused,docClick,docKeyDown",
    startGame$0: function() {
      var t1, message;
      this._clearDocEvents$0();
      t1 = H.setRuntimeTypeInfo(new W._EventStream(document, C.EventStreamProvider_keydown._eventType, false), [null]);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new S.Game_startGame_closure(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      t1 = J.get$onClick$x(this.level.canvas);
      H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new S.Game_startGame_closure0(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)])._tryResume$0();
      this._reset$0();
      message = document.querySelector("#message");
      if (message != null)
        J.remove$0$ax(message);
    },
    _update$0: function() {
      var head, t1, t2, x, y, tail, width, height;
      if (this.paused)
        return;
      head = C.JSArray_methods.get$first(this.snake.parts);
      t1 = this.snake;
      t2 = this.direction;
      t1 = t1.parts;
      x = C.JSArray_methods.get$first(t1).x;
      y = C.JSArray_methods.get$first(t1).y;
      if (t2 === $.Direction_right)
        ++x;
      else if (t2 === $.Direction_left)
        --x;
      else if (t2 === $.Direction_up)
        --y;
      else if (t2 === $.Direction_down)
        ++y;
      tail = C.JSArray_methods.get$last(t1);
      if (0 >= t1.length)
        return H.ioore(t1, 0);
      t1.pop();
      tail.x = x;
      ...
      t1.splice(0, 0, tail);
      t1 = this.snake;
      width = J.get$width$x(t1.level.canvas);
      height = J.get$height$x(t1.level.canvas);
      t1 = t1.parts;
      x = C.JSArray_methods.get$first(t1).x;
      y = C.JSArray_methods.get$first(t1).y;
      ...
      if (x < width / 20)
        ...
        t1 = true;
      if (t1 || this.snake.get$tailCollision()) {
        if (this.over === 0) {
          this.interval.cancel$0();
          S.Message$("Game Over", "press any key or click to try again");
          this._clearDocEvents$0();
          this._initStartEvents$0();
        }
        ++this.over;
      }
      t1 = this.snake;
      t2 = this.food;
      t1 = t1.parts;
      if (C.JSArray_methods.get$first(t1).x === t2.x && C.JSArray_methods.get$first(t1).y === t2.y) {
        this.food = S.Food$(this.level);
        t1 = head.x;
        t2 = head.y;
        this.snake.parts.push(new S.Point(t1, t2));
        t1 = this.speed;
        if (t1 <= 40)
          this.speed = t1 + 1;
        this.interval.cancel$0();
        this.interval = P.Timer_Timer$periodic(P.Duration$(0, 0, 0, C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0(1000 / this.speed)), 0, 0), new S.Game__update_closure(this));
        t1 = this.score + 3 * (this.speed - 15 + 1);
        this.score = t1;
        this.scoreBoard.textContent = "Score: " + C.JSInt_methods.toString$0(t1);
      }
    },
    ...
    _reset$0: function() {
      ...
      this.over = 0;
      this.speed = 15;
      var t1 = this.interval;
      if (t1 != null)
        t1.cancel$0();
      this.interval = P.Timer_Timer$periodic(P.Duration$(0, 0, 0, C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0(1000 / this.speed)), 0, 0), new S.Game__reset_closure(this));
      this.score = 0;
    },
    _initStartEvents$0: function() {
      var t1 = H.setRuntimeTypeInfo(new W._EventStream(document, C.EventStreamProvider_click._eventType, false), [null]);
      t1 = H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new S.Game__initStartEvents_closure(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)]);
      t1._tryResume$0();
      this.docClick = t1;
      t1 = H.setRuntimeTypeInfo(new W._EventStream(document, C.EventStreamProvider_keypress._eventType, false), [null]);
      t1 = H.setRuntimeTypeInfo(new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new S.Game__initStartEvents_closure0(this)), t1._useCapture), [H.getTypeArgumentByIndex(t1, 0)]);
      t1._tryResume$0();
      this.docKeyDown = t1;
    },
    _clearDocEvents$0: function() {
      var t1 = this.docClick;
      if (t1 != null)
        t1.cancel$0();
      t1 = this.docKeyDown;
      if (t1 != null)
    ...
    Game$0: function() {
      var t1, t2;
      document.body.appendChild(W.Element_Element$html("      <div id=\"main\">\r\n        <div id=\"header\">\r\n          <span id=\"score-board\"></span>\r\n        </div>\r\n        <div id=\"content\"></div>\r\n      </div>\r\n    ", null, null));
      this.scoreBoard = document.querySelector("#score-board");
      t1 = document.querySelector("#content");
      this.content = t1;
      t2 = new S.Level(t1, null, null);
      t2.Level$1(t1);
      this.level = t2;
      S.Message$("Welcome to Snake Dart", "press any key or click to start");
      this._initStartEvents$0();
    }
  },
  Game_startGame_closure: {
    ...
    call$1: function(e) {
      ...
      if (key === $.Direction_left || key === $.Direction_up || key === $.Direction_right || key === $.Direction_down) {
        this.this_0.direction = key;
        e.preventDefault();
      } else {
        t1 = e.keyCode;
        if (t1 === $.KeyCode_enter || t1 === $.KeyCode_space) {
          t1 = this.this_0;
          t1.paused = !t1.paused;
        }
      }
  ...
  Game__initStartEvents_closure: {
    "^": "Closure:9;this_0",
    call$1: function(e) {
      return this.this_0.startGame$0();
    }
  },
  Game__initStartEvents_closure0: {
    "^": "Closure:9;this_1",
    call$1: function(e) {
      return this.this_1.startGame$0();
    }
  },
  Point: {
    "^": "Object;x,y"
  ...
  Level: {
    ...
    Level$1: function(target) {
      ...
      J.set$width$x(this.canvas, C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(t1.clientWidth)));
      J.set$height$x(this.canvas, C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(t1.clientHeight)));
      this.render$0();
    }
  },
  Snake: {
    ...
    render$0: function() {
      var context, t1, t2, i, s;
      context = this.level.context;
      for (t1 = this.parts, t2 = J.getInterceptor$x(context), i = 0; i < t1.length; ++i) {
        s = t1[i];
        t2.set$fillStyle(context, "green");
        context.fillRect(s.x * 20, s.y * 20, 20, 20);
      }
    },
    get$tailCollision: function() {
      var t1, i, part;
      for (t1 = this.parts, i = 1; i < t1.length; ++i) {
        part = t1[i];
        if (C.JSArray_methods.get$first(t1).x === part.x && C.JSArray_methods.get$first(t1).y === part.y)
          return true;
      }
      return false;
    },
    Snake$1: function(level) {
      var t1, i;
      for (t1 = this.parts, i = 7; i >= 0; --i)
        t1.push(new S.Point(i, 0));
    },
    static: {Snake$: function(level) {
        var t1 = new S.Snake(level, []);
        t1.Snake$1(level);
        return t1;
      }}
  },
  Food: {
    "^": "Point;level,x,y",
    Food$1: function(level) {
      var t1 = J.get$width$x(this.level.canvas);
      if (typeof t1 !== "number")
        return t1.$sub();
      this.x = C.C__JSRandom.nextInt$1(C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0((t1 - 20) / 20)));
      t1 = J.get$height$x(this.level.canvas);
      if (typeof t1 !== "number")
        return t1.$sub();
      this.y = C.C__JSRandom.nextInt$1(C.JSNumber_methods.toInt$0(C.JSInt_methods.roundToDouble$0((t1 - 20) / 20)));
    },
    static: {Food$: function(level) {
        var t1 = new S.Food(level, 0, 0);
        t1.Food$1(level);
        return t1;
      }}
  },
  Message: {
    "^": "Object;",
    Message$2: function(title, subTitle) {
      document.body.appendChild(W.Element_Element$html("      <div id=\"message\">\r\n        <div id=\"message-modal\"></div>\r\n        <div id=\"message-panel\"><h1>" + title + "</h1><br/><h4>" + subTitle + "</h4></div>\r\n      </div>\r\n    ", null, null));
    },
    static: {Message$: function(title, subTitle) {
        var t1 = new S.Message();
        t1.Message$2(title, subTitle);
        return t1;
      }}
  }
},
...
["dart.dom.html", "dart:html", , W, {
  ...
  NodeValidatorBuilder_allowsAttribute_closure: {
    "^": "Closure:9;element_0,attributeName_1,value_2",
    call$1: function(v) {
...
function dart_precompiled($collectedClasses) {
  ...
  function Game(scoreBoard, content, level, snake, food, score, speed, direction, interval, over, paused, docClick, docKeyDown) {
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game.prototype = $desc;
  function Game_startGame_closure(this_0) {
    this.this_0 = this_0;
  }
  Game_startGame_closure.builtin$cls = "Game_startGame_closure";
  if (!"name" in Game_startGame_closure)
    Game_startGame_closure.name = "Game_startGame_closure";
  $desc = $collectedClasses.Game_startGame_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game_startGame_closure.prototype = $desc;
  function Game_startGame_closure0(this_1) {
    this.this_1 = this_1;
  }
  Game_startGame_closure0.builtin$cls = "Game_startGame_closure0";
  if (!"name" in Game_startGame_closure0)
    Game_startGame_closure0.name = "Game_startGame_closure0";
  $desc = $collectedClasses.Game_startGame_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game_startGame_closure0.prototype = $desc;
  function Game__update_closure(this_0) {
    this.this_0 = this_0;
  }
  Game__update_closure.builtin$cls = "Game__update_closure";
  if (!"name" in Game__update_closure)
    Game__update_closure.name = "Game__update_closure";
  $desc = $collectedClasses.Game__update_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__update_closure.prototype = $desc;
  function Game__renderAll_closure(this_0) {
    this.this_0 = this_0;
  }
  Game__renderAll_closure.builtin$cls = "Game__renderAll_closure";
  if (!"name" in Game__renderAll_closure)
    Game__renderAll_closure.name = "Game__renderAll_closure";
  $desc = $collectedClasses.Game__renderAll_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__renderAll_closure.prototype = $desc;
  function Game__reset_closure(this_0) {
    this.this_0 = this_0;
  }
  Game__reset_closure.builtin$cls = "Game__reset_closure";
  if (!"name" in Game__reset_closure)
    Game__reset_closure.name = "Game__reset_closure";
  $desc = $collectedClasses.Game__reset_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__reset_closure.prototype = $desc;
  function Game__initStartEvents_closure(this_0) {
    this.this_0 = this_0;
  }
  Game__initStartEvents_closure.builtin$cls = "Game__initStartEvents_closure";
  if (!"name" in Game__initStartEvents_closure)
    Game__initStartEvents_closure.name = "Game__initStartEvents_closure";
  $desc = $collectedClasses.Game__initStartEvents_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__initStartEvents_closure.prototype = $desc;
  function Game__initStartEvents_closure0(this_1) {
    this.this_1 = this_1;
  }
  Game__initStartEvents_closure0.builtin$cls = "Game__initStartEvents_closure0";
  if (!"name" in Game__initStartEvents_closure0)
    Game__initStartEvents_closure0.name = "Game__initStartEvents_closure0";
  $desc = $collectedClasses.Game__initStartEvents_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Game__initStartEvents_closure0.prototype = $desc;
  function Point(x, y) {
    this.x = x;
    this.y = y;
  }
  ...
  function Level(target, canvas, context) {
    this.target = target;
    this.canvas = canvas;
  ...
  function Snake(level, parts) {
    this.level = level;
    this.parts = parts;
  }
  Snake.builtin$cls = "Snake";
  if (!"name" in Snake)
    Snake.name = "Snake";
  $desc = $collectedClasses.Snake;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Snake.prototype = $desc;
  function Food(level, x, y) {
    this.level = level;
    this.x = x;
    this.y = y;
  }
